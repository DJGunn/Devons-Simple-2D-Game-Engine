...
MenuButton::MenuButton(int xPosition, int yPosition, string text, GameStates state)
{
	...
	//Error check
	//if (buttonSpriteSheet == NULL) cerr << "Could not load MenuButton's sprite sheet." << endl;
	...
	//Error check
	//if (buttonSurfaceText == NULL) cerr << "Could not create MenuButton's button-text." << endl;
...
int MenuButton::height()
{
	return button.h;
}

int MenuButton::getXPos()
{
	return button.x + (button.w/2);
}

int MenuButton::getYPos()
{
